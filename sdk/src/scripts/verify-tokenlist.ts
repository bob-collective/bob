...
import { createPublicClient, http, isAddress, getAddress, parseAbi, Chain } from 'viem';
...
async function validateToken(token: Token): Promise<ValidationResult> {
    ...
    if (!isValidAddress) {
        issues.push(`Invalid or non-checksummed address, should be ${getAddress(token.address)}`);
    ...
    if (token.extensions && token.extensions['bridgeInfo']['1']) {
        const bridgeInfo = token.extensions['bridgeInfo']['1'];
        const l1TokenAddress = bridgeInfo['tokenAddress'];
        if (l1TokenAddress) {
            if (!validateTokenAddress(l1TokenAddress)) {
                issues.push(
                    `Invalid L1 token address in bridgeInfo: ${l1TokenAddress}, should be ${getAddress(l1TokenAddress)}`
                );
            }
            ...
            if (token.name !== onChainL1Data.name) {
                issues.push(`Name mismatch: tokenlist="${token.name}", onchain="${onChainL1Data.name}"`);
            }

            if (token.symbol !== onChainL1Data.symbol) {
                issues.push(`Symbol mismatch: tokenlist="${token.symbol}", onchain="${onChainL1Data.symbol}"`);
            }

            if (token.decimals !== onChainL1Data.decimals) {
                issues.push(`Decimals mismatch: tokenlist=${token.decimals}, onchain=${onChainL1Data.decimals}`);
            }
        }
    }
...
async function main() {
    ...
    const { tokens }: { tokens: Token[] } = JSON.parse(fs.readFileSync(tokenlistPath, 'utf8'));
