...
export class GatewayApiClient {
		...
		return this.api.getQuote(
			{
				...
				sender: params.fromUserAddress
					? params.fromUserAddress?.toString()
					: (null as any),
				// eslint-disable-next-line @typescript-eslint/no-explicit-any
				recipient: params.toUserAddress
					? params.toUserAddress.toString()
					: (null as any),
	...
	async executeQuote(
		{
			...
			publicClient,
			btcSigner,
		}: { quote: GatewayQuote } & AllWalletClientParams,
		initOverrides?: RequestInit,
			...
			if (needsApproval && !isAddressEqual(tokenAddress, zeroAddress)) {
				if (isAddressEqual(tokenAddress, WBTC_OFT_ADDRESS)) {
					...
					const { request } = await publicClient.simulateContract({
						account: walletClient.account,
						address: tokenAddress,
						abi: isAddressEqual(tokenAddress, ETHEREUM_USDT_ADDRESS)
							? USDTApproveAbi
							: erc20Abi,
						functionName: "approve",
						args: [spenderAddress, maxUint256],
	...
	async executeStrategy({
		...
		publicClient,
		...params
	}: StrategyParams & EvmWalletClientParams) {
		const allowance = await publicClient.readContract({
	...
	async getOrders(
		userAddress: Address,
		initOverrides?: RequestInit,
	): Promise<Array<GatewayOrderInfo>> {
