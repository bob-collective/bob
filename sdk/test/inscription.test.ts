...
describe.skip('Inscription Tests', () => {
    it('should parse text inscription', async () => {
        ...
        const txHex = await esploraClient.getTransactionHex(
        ...
        assert.deepStrictEqual(JSON.parse(content.toString('utf-8')), {
            p: 'brc-20',
            op: 'deploy',
            tick: 'ordi',
            max: '21000000',
    ...
    it('should parse image inscription', async () => {
        ...
        const txHex = await esploraClient.getTransactionHex(
            '79ddcce9b4aaa4d2c3ba512a1dfd9bf2dd1f840eab98101c41bf8b801bcb3e0c'
        );
        const tx = bitcoin.Transaction.fromHex(txHex);
