...
const TRON_ACCOUNT = 'TXYZopYRdj2D9XRtbG411XZZ3kM5VkAeBf';
const TRON_ACCOUNT_HEX = '0x41eca9bc828a3005b9a3b909f2cc5c2a54794de05f';
const TRON_TOKEN = 'TLa2f6VPqDgRE67v1736s7bJ8Ray5wYjU7';
const TRON_TOKEN_HEX = '0x4174472e7d35395a6b5add427eecb7f4b62ad2b071';
const TRON_SPENDER = 'TMwFHYXLJaRUPeW6421aqXL4ZEzPRFGkGT';
const TRON_SPENDER_HEX = '0x41834295921a488d9d42b4b3021ed1a3c39fb0f03e';
...
describe('Gateway Tests', () => {
    ...
    it('should execute tron offramp using hex addresses for public client calls', async () => {
        const gatewaySDK = new GatewaySDK();
        ...
        const mockQuote: GatewayQuoteV2OneOf1 = {
            offramp: {
                srcChain: 'tron',
                feeBreakdown: {
                    protocolFee: { address: TRON_TOKEN, amount: '5', chain: 'tron' },
                    affiliateFee: { address: TRON_TOKEN, amount: '2', chain: 'tron' },
                    solverFee: { address: TRON_TOKEN, amount: '1', chain: 'tron' },
                    inclusionFee: { address: TRON_TOKEN, amount: '1', chain: 'tron' },
                    fastestFeeRate: '6',
                },
                inputAmount: { address: TRON_TOKEN, amount: '1000', chain: 'tron' },
                outputAmount: { address: zeroAddress, amount: '990', chain: 'bitcoin' },
                tokenAddress: TRON_TOKEN,
                recipient: 'bc1qw508d6qejxtdg4y5r3zarvary0c5xw7kg3g4ty',
                slippage: 0,
                totalFeeUsd: '3',
                txTo: TRON_SPENDER,
            },
        ...
        const sendTransaction = vi.fn().mockResolvedValue('0xtronhash');
        const mockWalletClient = {
            account: { address: TRON_ACCOUNT },
            writeContract,
            sendTransaction,
        } as unknown as WalletClient<Transport, ViemChain, Account>;
        ...
        expect(result.tx).toBe('0xtronhash');
        expect(readContract).toHaveBeenCalledWith(
            expect.objectContaining({
                address: TRON_SPENDER_HEX,
        ...
        expect(multicall).toHaveBeenCalledWith(
            expect.objectContaining({
                contracts: [
                    expect.objectContaining({
                        address: TRON_TOKEN_HEX,
                        args: [TRON_ACCOUNT_HEX, TRON_SPENDER_HEX],
                    }),
                ],
            })
        );
        expect(simulateContract).toHaveBeenCalledWith(
            expect.objectContaining({
                account: TRON_ACCOUNT_HEX,
                address: TRON_TOKEN_HEX,
                args: [TRON_SPENDER_HEX, 1000n],
            })
        );
        expect(writeContract).toHaveBeenCalledWith(
            expect.objectContaining({
                account: expect.objectContaining({ address: TRON_ACCOUNT }),
                address: TRON_TOKEN,
                args: [TRON_SPENDER, 1000n],
            })
        );
    ...
    it('should execute tron token swap using hex addresses for public client calls', async () => {
        const mockedQuote: GatewayQuoteV2OneOf2 = {
            tokenSwap: {
                dstChain: 'bob',
                estimatedTimeInSecs: 60,
