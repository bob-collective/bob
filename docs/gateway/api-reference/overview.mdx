...
<Steps>
  ...
  <Step title="Monitor Single Order">
    Track the status of a specific order using [`GET /v1/get-order/{id}`](/api-reference/v1/get-all-orders-for-a-specific-id). This endpoint returns the status and details for a single order by its transaction hash.

    ```bash
    GET /v1/get-order/0x1234abcd...
    ```
  ...
  <Step title="Monitor All User Orders">
    Track the progress of all orders for a user using [`GET /v1/get-orders/{user_address}`](/api-reference/v1/get-all-user-orders). This endpoint returns every order (pending and completed) associated with the specified user address.
