...
<Steps>
  ...
  <Step title="Track settlement">
    ...
    ```bash
    gateway-cli orders --json
    ```
  </Step>
</Steps>

<Note>
  If an agent submits a transaction outside of the CLI (e.g. via a custom signer) and needs to register it for tracking, use `gateway-cli register --json`. This is the recovery path for agents that handle signing separately.
</Note>
