...
The core things that need building are:

- Adding the base EVM compatibility layer from Frontier, which then needs to be extended for OP Stack compatibility. Then, frontier would need to be extended to be compatible with OP Stack. First focus needs to be on the op-geth execution engine and then on the settlement/fraud proof contracts.
