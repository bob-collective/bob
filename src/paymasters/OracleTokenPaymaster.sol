...
contract OracleTokenPaymaster is BasePaymaster {
    ...
    event PostRelay(
        uint actualEthAmount,
        IERC20 token,
        uint actualTokenAmount,
        address payer
    );
    ...
    function _ethToTokens(
        ...
        require(
            address(_tokenOracle.oracle) != address(0x00),
    ...
    function _verifyPaymasterData(
        ...
        require(
            relayRequest.relayData.paymasterData.length == 64,
    ...
    {
        (IERC20 token, uint maxTokens) = _getPaymasterData(
        ...
        require(
            tokenPrecharge <= maxTokens,
            "Tx cost more than the user-supplied limit"
        );
    ...
    function _postRelayedCall(
        ...
        (address payer, uint256 tokenPrecharge, IERC20 token) = abi.decode(
            context,
            (address, uint256, IERC20)
        );
    ...
    function _postRelayedCallInternal(
        ...
        uint256 tokenActualCharge = _ethToTokens(
            token,
            valueRequested + ethActualCharge
        );
