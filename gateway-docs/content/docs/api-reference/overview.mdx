...
- **Optional slippage** — omit `slippage` from `GET /v4/get-quote` to let Gateway choose a suitable tolerance for the route. The quote reports the resolved value in basis points. Explicit values remain supported; V3 still requires the parameter. See the [recommendation example](/gateway/faq#what-if-i-dont-know-what-slippage-to-set).
