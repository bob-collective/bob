...
### Order creation
    ...
    Supply a Bitcoin `refundAddress` in the quote before creating a V4 order. `create-order` returns an `orderId` and a **unique Bitcoin deposit address belonging to the solver's wallet**. If you passed the user's `sender` address, the response also carries a ready-to-sign PSBT (`psbtHex`) with coin selection already done.
  </Step>
...
### Bitcoin payment
    ...
    The user signs one Bitcoin transaction. Your app submits the finalized raw transaction hex to `/v4/register-tx`; Gateway validates it against the order, screens it, and broadcasts it. A txid alone is not accepted. Nothing on the destination chain has happened yet — the user has spent only their BTC and the Bitcoin miner fee.
  </Step>
...
### If it can't land
    ...
    - for V4 orders, the required Bitcoin `refundAddress` determines the refund destination: `commitBitcoinRefund` reserves the order's terminal state on-chain and the BTC is then sent from the solver's Bitcoin wallet, net of the fee needed to confirm that transaction
    - for older orders created without a Bitcoin refund address, `refundOrder` pays USDT to the order owner on Ethereum
...
sequenceDiagram
    ...
    User->>API: register-tx (signed raw Bitcoin hex)
    API->>API: validate transaction + screen
    API->>Bitcoin: broadcast BTC deposit
...
### Quote and order creation
    ...
    The user signs and broadcasts the source-chain transaction. Gateway indexes it from the chain; there is no V4 offramp `register-tx` call.
