...
V4 is the current BOB Gateway API. It requires a source-chain refund address before order creation and makes `register-tx` an onramp-only Bitcoin submission endpoint. V3 remains available for existing integrations.
...
### 3. Submit only signed Bitcoin transactions to register-tx
...
### 4. Keep shared response handling
