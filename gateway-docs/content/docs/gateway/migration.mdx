...
Slippage is expressed in basis points (`100` = 1%, `300` = 3%). Read the resolved value from `onramp.slippage`, `offramp.slippage`, or `tokenSwap.slippage` and show it before execution. Pass the returned quote unchanged to `create-order`; to use a different tolerance, request a new quote with an explicit `slippage` value. See the [recommendation example](/gateway/faq#what-if-i-dont-know-what-slippage-to-set).
