...
- **Token swaps that stay on one chain use at most 0.15%** and honor a tighter explicit request. None of the guidance above applies to them, and the quote will tell you the figure actually used.

### Can I use automated slippage if I don't know what to set?

Yes. Omit `slippage` from `GET /v4/get-quote`, or omit `maxSlippage` when calling the SDK's `getQuote`. Gateway chooses a suitable tolerance for the route and includes it in the quote. You can request and inspect this recommendation without creating an order.
