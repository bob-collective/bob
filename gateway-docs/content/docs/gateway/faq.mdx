...
If you don't have a tolerance in mind, let Gateway recommend one. To set your own limit, pass `slippage` to the V4 API or `maxSlippage` to the SDK in basis points: `100` = 1%, `300` = 3%. An explicit `0` means zero tolerance; it does not request a recommendation.
...
Review the recommended tolerance before creating an order. See [Request a Slippage Recommendation](/gateway/integration#request-a-slippage-recommendation) for SDK and API examples, and the [migration notes](/gateway/migration#migrating-the-sdk) if you use an older SDK.
