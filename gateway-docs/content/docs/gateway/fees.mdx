...
You configure affiliate fees **per quote** — there's no upfront registration. Pass one or more `{ address, bps }` pairs when you request a quote, and the fee is deducted as part of the swap and paid to your address(es) on settlement. Onramp and offramp quotes support splitting the fee across multiple recipients in a single quote — useful for revenue splits between an aggregator and an underlying integrator, referral programs, or multi-party agreements.
