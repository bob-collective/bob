...
|---|---|---|
| BTC to X: swap can't fill at quoted terms | BTC to their Bitcoin refund address | Gateway, automatic |
| BTC to X: destination swap or bridge can't land | BTC to their Bitcoin refund address | Gateway, automatic |
...
Both cases resolve automatically, with no refund transaction for the user or integrator to submit. The solver simulates the destination call before releasing, then re-quotes and retries with exponential backoff — starting at 15 seconds and doubling — for up to an hour. If the release still can't land, Gateway commits the order to a Bitcoin refund on-chain and pays the order's Bitcoin `refundAddress` from the solver's wallet. The Bitcoin miner fee is deducted from the refund.

A refunded order earns no fee. Every fee withheld from the fill — solver, protocol and affiliate — is returned along with the deposit.

## Set the refund address before execution

V4 requires `refundAddress` before order creation for **all routes**. It may be omitted for a price-only quote, but `create-order` rejects such a quote with `MISSING_REFUND_ADDRESS`. Request a fresh quote with the address before execution.

- For an onramp, use the user's Bitcoin refund address.
- For an offramp or token swap, use an address encoded for the source chain.
- The SDK forwards `refundAddress`; it does not infer it from the sender.

Older onramp orders created without a Bitcoin refund address can settle refunds in USDT on Ethereum to the order owner. New V4 orders require the address; see the [Migration Guide](/gateway/migration).
...
A completed refund shows up as the `refunded` order status, which lists the `refundedTokens` actually returned in the SDK (`refunded_tokens` in raw API JSON) — each with `chain`, `token`, `amount`, and the settlement `txHash`:
