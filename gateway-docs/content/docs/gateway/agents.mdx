...
<Callout>
  The CLI currently uses the V3 SDK/API. For a direct V4 integration, submit signed raw Bitcoin transaction hex to `PATCH /v4/register-tx` for onramps; a txid alone is not accepted. Offramp and token-swap transactions are indexed from the source chain. See the [V4 migration notes](/gateway/migration).
</Callout>
